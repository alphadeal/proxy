...
import { startProxy, getProxyConfigPath } from "./standalone-proxy.js";
...
async function checkForUpdate(): Promise<string | null> {
    try {
        ...
        const res = await fetch(
            "https://registry.npmjs.org/@relayplane/proxy/latest",
            {
...
async function handleLoginCommand(): Promise<void> {
    ...
    try {
        ...
        if (!startRes.ok) {
            console.error(
                "  ❌ Failed to start login flow. Is the API reachable?",
            );
            process.exit(1);
        }
        ...
        console.log("");
        console.log(
            `  Waiting for approval (expires in ${Math.floor(expiresIn / 60)} minutes)...`,
        );
        ...
        while (Date.now() < deadline) {
            await new Promise((r) =>
                setTimeout(r, (pollIntervalSec || 5) * 1000),
            );
            ...
            if (pollData.status === "approved") {
                ...
                console.log("");
                console.log(
                    "  ☁️  Cloud sync will activate on next proxy start.",
                );
                console.log("");
                return;
            }
...
async function handleCloudStatusCommand(): Promise<void> {
    ...
    console.log(
        `  Proxy:       ${proxyReachable ? "🟢 Running" : "🔴 Stopped"}`,
    );

    // Auth status
    if (creds?.apiKey) {
        console.log(
            `  Account:     ✅ Logged in${creds.email ? ` (${creds.email})` : ""}`,
        );
...
function handleStatsCommand(): void {
    ...
    if (stats.totalEvents === 0) {
        console.log(
            "  No data yet. Start using the proxy to collect statistics.",
        );
        console.log("");
    }
}
...
async function handleMeshCommand(args: string[]): Promise<void> {
    ...
    if (sub === "status") {
        ...
        try {
            const controller = new AbortController();
            const timeout = setTimeout(() => controller.abort(), 2000);
            const res = await fetch("http://127.0.0.1:4100/v1/mesh/status", {
                signal: controller.signal,
            });
            clearTimeout(timeout);
            if (res.ok) {
                ...
                console.log(
                    `  Contributing:  ${m.contributing ? "✅ Yes (sharing with mesh)" : "❌ No (local only)"}`,
                );
    ...
    if (sub === "sync") {
        try {
            ...
            if (res.ok) {
                const data = (await res.json()) as {
                    sync: { pushed?: number; pulled?: number; error?: string };
                };
                if (data.sync.error) {
        ...
        try {
            ...
            if (res.ok) {
                ...
                if (data.tips.length === 0) {
                    console.log(
                        "No tips yet. Use the proxy to build knowledge.",
                    );
                    return;
                }
                console.log("");
                    ...
                    const icon =
                        tip.type === "tool"
                            ...
                            : tip.type === "negative"
                              ...
                              : "💡";
                    console.log(
                        `  ${icon} [${tip.fitness.toFixed(2)}] ${tip.observation}`,
                    );
                }
                console.log("");
            } else {
    ...
    if (sub === "contribute") {
        ...
        if (!value || value === "status") {
            ...
            if (enabled) {
                console.log(
                    "  You are sharing anonymized routing data with the collective mesh.",
                );
                console.log(
                    "  This improves routing for everyone on the network.",
                );
            ...
            console.log("");
            console.log(
                "  Never shared: prompts, responses, file paths, API keys",
            );
            console.log("");
            return;
        }
        ...
        if (value === "on") {
            ...
            console.log(
                "  Anonymized routing data will be shared with the collective mesh.",
            );
        ...
        if (value === "off") {
            ...
            console.log(
                "  Your data stays local. Restart the proxy for changes to take effect.\n",
            );
            return;
        }
    ...
    console.log(
        "Unknown mesh subcommand. Available: status, sync, tips, contribute",
    );
}
...
async function main(): Promise<void> {
    ...
    let anthropicAuth: "auto" | "passthrough" | "env" = "auto";
    ...
    for (let i = 0; i < args.length; i++) {
        ...
        if (arg === "--port" && args[i + 1]) {
            ...
            offline = true;
        } else if (arg === "--anthropic-auth" && args[i + 1]) {
            const val = args[i + 1]!;
            if (val === "auto" || val === "passthrough" || val === "env") {
                anthropicAuth = val;
            } else {
                console.error(
                    `Error: Invalid --anthropic-auth value "${val}". Must be one of: auto, passthrough, env`,
                );
                process.exit(1);
            }
            i++;
        } else if (arg?.startsWith("--anthropic-auth=")) {
            const val = arg.slice("--anthropic-auth=".length);
            if (val === "auto" || val === "passthrough" || val === "env") {
                anthropicAuth = val;
            } else {
                console.error(
    ...
    // Check for at least one API key (or a valid keyless auth mode)
    ...
    // Also check config file for MAX token or passthrough mode
    const proxyConfigPath = getProxyConfigPath();
    let hasMaxToken = false;
    try {
        const raw = await fs.promises.readFile(proxyConfigPath, "utf8");
        const fileCfg = JSON.parse(raw) as {
            auth?: { anthropicMaxToken?: string };
        };
        hasMaxToken = !!fileCfg?.auth?.anthropicMaxToken;
    } catch {
        // config file absent or unreadable — that's fine
    }

    // passthrough mode: proxy forwards the client's own auth, no env key needed
    const isPassthrough = anthropicAuth === "passthrough";
    ...
    if (
        ...
        console.error(
            "  ANTHROPIC_API_KEY, OPENAI_API_KEY, GEMINI_API_KEY, XAI_API_KEY, OPENROUTER_API_KEY, DEEPSEEK_API_KEY, GROQ_API_KEY",
        );
        console.error("");
        console.error(
            "Alternatively, if you are using a Claude Max account (OAuth token), either:",
        );
        console.error(
            "  • Add your token to ~/.relayplane/config.json under auth.anthropicMaxToken",
        );
        console.error(
            "  • Start with --anthropic-auth=passthrough to forward your client's auth automatically",
        );
        process.exit(1);
    }
    ...
    if (creds?.apiKey && !offline) {
        console.log(
            `    ☁️  Cloud sync: active (plan: ${creds.plan || "free"})`,
        );
    } else if (!creds?.apiKey) {
        console.log(
            "    💻 Local only (run `relayplane login` for cloud sync)",
        );
    }

    console.log("");
    console.log("  Providers:");
    if (hasAnthropicKey) console.log("    ✓ Anthropic (API key)");
    if (hasMaxToken) console.log("    ✓ Anthropic (MAX subscription token)");
    if (isPassthrough)
        console.log("    ✓ Anthropic (passthrough — forwarding client auth)");
    ...
    try {
        await startProxy({ port, host, verbose, anthropicAuth });
